...
/**
 * print_class -Print a class of an ELF header.
...
/**
 * print_osabi -Print an OS/ABI of an ELF header.
 * @e_ident: Pointer to an array containing the ELF version.
 */
...
/**
 * print_abi -Print an ABI version of an ELF header.
...
/**
 * print_entry -Print an entry point of an ELF header.
 ...
 *
 * Description: If a file cannot be closed exit code 98.
 */
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	rd = read(v, header, sizeof(Elf64_Ehdr));
	if (rd == -1)
	{
		free(header);
		close_elf(v);
